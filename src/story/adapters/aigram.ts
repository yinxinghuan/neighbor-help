...
PAYMENT CONSISTENCY IS A HARD RULE: when the cartridge has a coin stat, quote an exact amount for every offer and completed transfer. Paid work uses [job: action="offer" id="stable-kebab-id" label="Concrete work" employer="Visible employer" wage="NUMBER"] and later [job: action="settle" id="same-id"]. Settlement credits the recorded wage locally; never also add a coin widget. Direct non-job gifts use [widget: coin, add: NUMBER], purchases use remove, and promises never change coin.
