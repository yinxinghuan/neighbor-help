...
function ConversationHeader({ cartridge, engine, audio, openWorld, openBoard, textSize, setTextSize }: {
  cartridge: StoryCartridge; engine: ReturnType<typeof useStoryEngine>; audio: ReturnType<typeof useStoryAudio>; openWorld: (active?: DrawerId, detail?: WorldDetail) => void; openBoard: () => void; textSize: TextSize; setTextSize: (size: TextSize) => void
}) {
  ...
  return <header className="st-chat-header">
    <div className="st-chat-header__top">
      ...
      <div className="st-chat-header__actions">
        ...
        <button className="st-board-button" onClick={openBoard} aria-label={cartridge.locale === 'zh' ? '共享公告' : 'Shared board'} title={cartridge.locale === 'zh' ? '共享公告' : 'Shared board'}><Icon name="board" /></button>
...
function Game({ cartridge, mode, chatId, onSelect, onLocaleChange }: { cartridge: StoryCartridge; mode: StoryMode; chatId?: string; onSelect: (id: string) => void; onLocaleChange: (locale: Locale) => void }) {
  ...
  const neighborhood = useNeighborhoodWorld(player)
  ...
  const advanceStory = (action: string) => {
  ...
  const recoverSharedAction = (action: string) => {
    const code = neighborhood.notice?.kind === 'error' ? neighborhood.notice.code : undefined
    engine.recordSharedWorldRecovery(action, sharedConflictNarrative(cartridge.locale, code), sharedRecoveryChoices(cartridge.locale))
    audio.cue('error')
  }
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (index >= 0 && index < engine.save.choices.length && !(event.target instanceof HTMLInputElement)) void act(engine.save.choices[index].label)
    }
  ...
  return <main className={`st-shell st-shell--${cartridge.theme.material}`} data-text-size={textSize} style={setCssTheme(cartridge)}>
    <ConversationHeader cartridge={cartridge} engine={engine} audio={audio} openWorld={openWorld} openBoard={() => setBoardOpen(true)} textSize={textSize} setTextSize={setTextSize} />
    ...
    <Composer cartridge={cartridge} engine={engine} onAct={(action) => { void act(action) }} />
    ...
    {boardOpen && <NeighborhoodBoard world={neighborhood} locale={cartridge.locale} close={() => setBoardOpen(false)} onWorldAction={onBoardAction} />}
  </main>
}
