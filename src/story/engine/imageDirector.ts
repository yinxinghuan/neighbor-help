...
function importantDialogueOwner(next: StorySave, parsed: ParsedScene) {
...
function detectTriggers(previous: StorySave, next: StorySave, parsed: ParsedScene): SceneImageTrigger[] {
  ...
  if (importantDialogueOwner(next, parsed)) triggers.push('character-expression')
...
function focusFor(reason: SceneImageTrigger | 'cadence', parsed: ParsedScene, next: StorySave): string {
  ...
  if (reason === 'character-expression') {
    const owner = importantDialogueOwner(next, parsed)
...
function buildScenePrompt(
  ...
  const expression = reason === 'character-expression' ? importantDialogueOwner(next, parsed) : undefined
  return [
    ...
    expression ? `${expression.character?.name ?? expression.dialogue.speaker} is the one dominant visible adult. Use a medium close-up or chest-up reaction shot; make ${expression.expression ? `this expression visually specific: ${expression.expression}` : 'the current expression readable through eyes, mouth, posture and one restrained hand gesture'}. Keep enough current-location background for context and avoid a centered passport portrait.` : '',
...
export function chooseSceneImage(
  ...
  if (director && owner && director.guaranteedTriggers.includes('character-expression')) {
    return { prompt: buildScenePrompt(cartridge, next, parsed, 'character-expression', undefined, false), source: 'director', reason: 'character-expression', playerVisible: false }
  }
