...
一个可见按钮只能映射一个权威阶段。`claim_request` 的文案不得同时承诺送达或完成；`complete_request` 只能在最新快照仍为本人 `claimed` 时显示。成功响应必须把提交后的实体快照交给故事桥接层，不能只回传布尔成功后继续使用提交前实体。
...
成功提交后，已提交事件是不可逆的界面事实。叙事、图片或一致性校验失败时，客户端按事件类型渲染确定性结果，并从刷新后的请求状态重新建立选择；禁止声称行动没有发生、恢复提交前正文或再次提供同一完成按钮。旧存档或别的客户端推进世界后，若当前选择与请求阶段冲突，先写入简短状态校正，再移除失效动作。
...
9. 领取成功：正文仍在领取地点，只描述领取和保管；送达作为下一项独立动作。
10. 完成成功后故意让生成/校验失败：正文仍确认完成，地点和公共物品与权威快照一致，同一完成按钮不再出现。
11. 旧存档仍保留完成按钮但共享请求已完成：重进后自动移除旧动作，不能重复提交或进入通用失败循环。
